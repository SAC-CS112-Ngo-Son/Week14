...
public class Lab2
{
	...
	public static void main(String[] args)
	{
		...
		char cResult;
		boolean continueLoop = true, continueProgramLoop = true;
				...
				do
				{
					...
					do
					{
						aryGuess[iRound] = (JOptionPane.showInputDialog(
								String.format("Round %d: What's your guess? (L for Low and H for High)", iRound)))
										.charAt(0);
					} while ((aryGuess[iRound] != 'L') && (aryGuess[iRound] != 'H'));

					cResult = game.Low_Or_High();

					System.out.println(cResult); // tracking and matching
													// results
					...
					if (aryGuess[iRound] == cResult) // win case
					{
						...
						iChoice = JOptionPane.showConfirmDialog(null, "Your guess is correct. Do you want to continue?",
					...
					} else // lose case
					{
						...
						iChoice = JOptionPane.showConfirmDialog(null,
								"Your guess is incorrect. Do you want to continue?", null, JOptionPane.YES_NO_OPTION);
					}
				} while (iChoice == JOptionPane.YES_OPTION); // keep playing if
																// the
																// user
																// chooses YES
